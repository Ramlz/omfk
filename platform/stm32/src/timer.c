 ...
 */
static void timer_enable(timer_iface *iface, bool enabled) {
    timer_ctx *ctx = (timer_ctx*) iface;
    if (!ctx) {
        return;
    }
 ...
 */
static bool timer_iface_init(timer_iface *iface, const timer_config *config) {
    timer_ctx *ctx = (timer_ctx*) iface;
    if (!ctx || !config) {
        return false;
    }
