...
} __attribute__((packed,aligned(1))) pcd8544_cmd_type;
...
static void pcd8544_byte_cycle_write(pcd8544_ctx *ctx, uint8_t data) {
    for (uint8_t i = 0; i < 8; i++)  {
        ...
        ctx->io_iface->write(ctx->io_iface, ctx->clk_pin, true);
        ctx->io_iface->write(ctx->io_iface, ctx->clk_pin, false);
    }
}
...
static void pcd8544_reset(pcd8544_ctx *ctx) {
    ctx->io_iface->write(ctx->io_iface, ctx->rst_pin, false);
    ctx->io_iface->write(ctx->io_iface, ctx->rst_pin, true);
}
...
static void pcd8544_init_data_transmit(pcd8544_ctx *ctx) {
    ctx->io_iface->write(ctx->io_iface, ctx->dc_pin, true);
}
...
static void pcd8544_start_data_transmit(pcd8544_ctx *ctx) {
    ctx->io_iface->write(ctx->io_iface, ctx->ce_pin, false);
}
...
static void pcd8544_end_data_transmit(pcd8544_ctx *ctx) {
    ctx->io_iface->write(ctx->io_iface, ctx->ce_pin, true);
}
...
static void pcd8544_init_cmd_transmit(pcd8544_ctx *ctx) {
    ctx->io_iface->write(ctx->io_iface, ctx->dc_pin, false);
}
